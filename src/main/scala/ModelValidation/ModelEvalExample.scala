...
object ModelEvalExample {
  def main(args : Array[String]) {
    Logger.getLogger("org").setLevel(Level.ERROR)

    val spark = SparkSession.builder()
      .appName("ModelEvalExample")
      .master("local[*]")
      .getOrCreate()

    // Prepare training and test data.
    val data = spark.read.format("libsvm").load("src/main/scala/ModelValidation/sample_linear_regression_data.txt")
