...
object PopularMoviesDataSets {
  ...
  def loadMovieNames() : Map[Int, String] = {
    ...
    val lines = Source.fromFile("src/main/scala/DataAnalysis/ml-100k/u.item").getLines()
    for (line <- lines) {
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("src/main/scala/DataAnalysis/ml-100k/u.data").map(x => Movie(x.split("\t")(1).toInt))
